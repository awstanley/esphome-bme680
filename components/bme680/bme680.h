...
class BME680Component : public PollingComponent, public i2c::I2CDevice {
 ...
 protected:
  ...
  /// Calculate the gas resistance in Ω using the provided raw ADC value (BME680).
  uint32_t calc_gas_resistance_low_(uint16_t raw_gas, uint8_t range);
  /// Calculate the gas resistance in Ω using the provided raw ADC value (BME688).
  uint32_t calc_gas_resistance_high_(uint16_t raw_gas, uint8_t range);
  ...
  /// Used to detect BME680 (0) and BME688 (1).
  uint8_t gas_variant_;
};
